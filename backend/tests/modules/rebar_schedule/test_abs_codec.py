...
def test_a_block_ends_where_an_at_sign_is_followed_by_an_uppercase_letter() -> None:
    """The rule the standard states, exercised on a header full of uppercase values."""
    parsed = parse_record(spec_examples.VERIFIED["bf2d-8-coupler-and-thread"])
    assert [block.kind for block in parsed.blocks] == ["H", "G", "M", "C"]
    coupler = parsed.block("M")
    # 'bA12' holds an uppercase letter inside a value, which must not end the
    # block, and 'nLenton' is the next field rather than a new block.
...
def test_a_field_identifier_means_different_things_in_different_blocks() -> None:
    ...
    assert parsed.header_value("r") == "417"
    assert parsed.geometry.values("r") == ("400",)
...
def test_an_empty_field_still_carries_its_identifier() -> None:
    ...
    parsed = parse_record(spec_examples.VERIFIED["bf2d-1"])
...
def test_repeated_identifiers_are_kept_in_order() -> None:
    ...
    parsed = parse_record(spec_examples.VERIFIED["bf3d-1"])
    assert parsed.geometry.values("x") == ("294", "0", "0", "0", "294")


def test_a_mesh_geometry_block_carries_the_axis_whose_bars_are_bent() -> None:
    parsed = parse_record(spec_examples.VERIFIED["bfma-3-bent-drawn-mesh"])
    assert parsed.geometry.axis == "y"
    assert parsed.geometry.first("l") == "200"


def test_a_spatial_geometry_reads_its_first_coordinate_as_a_field_not_an_axis() -> None:
    """``Gx294@`` in a BF3D record is the field x, not the axis marker Gx.
    ...
    assert parsed.geometry.first("x") == "294"


def test_the_private_block_is_kept_whole() -> None:
    """It is free-form, so its content is not forced into identified fields."""
    record = "BF2D@HjTest@r1@ia@p1@l100@n1@e0.1@d12@gB500A@s48@v@Gl100@w0@Pcost-centre-4711@lot-12@"
...
def test_straight_legs_sum_to_the_length_the_header_states() -> None:
    parsed = parse_record(spec_examples.VERIFIED["bf2d-3-cranked"])
    segments = read_segments(parsed.geometry)
    assert [seg.length_mm for seg in segments] == [Decimal(n) for n in (100, 300, 424, 300, 100)]
...
def test_an_arc_contributes_the_length_of_the_curve_it_draws() -> None:
    """400 mm radius through 90 degrees is 628.3 mm of steel, not 400."""
    parsed = parse_record(spec_examples.VERIFIED["bf2d-4-arc"])
    segments = read_segments(parsed.geometry)
    assert [seg.radius_mm for seg in segments] == [None, Decimal(400), None]
...
def test_a_second_angle_after_an_arc_is_read_as_the_bend_into_the_next_leg() -> None:
    """``r400@w90@w45@`` is an arc opening 90 degrees, then a 45 degree bend."""
    parsed = parse_record(spec_examples.VERIFIED["bf2d-5-arc-opening-angle"])
    arc = read_segments(parsed.geometry)[1]
    assert arc.radius_mm == Decimal(400)
...
def test_a_spatial_bar_reads_as_one_offset_per_vertex() -> None:
    ...
    assert read_coordinates(parsed.geometry) == [
        (Decimal(294), Decimal(0), Decimal(0)),
        (Decimal(0), Decimal(288), Decimal(0)),
        (Decimal(0), Decimal(0), Decimal(288)),
        (Decimal(0), Decimal(-288), Decimal(0)),
        (Decimal(294), Decimal(0), Decimal(0)),
    ]
...
def test_a_helix_reads_as_a_planar_shape_and_its_turn_pitch_pairs() -> None:
    ...
    parsed = parse_record(spec_examples.VERIFIED["bfwe-1-square-column"])
    assert [seg.length_mm for seg in read_segments(parsed.geometry)] == [Decimal(340)] * 4
...
def test_a_round_helix_is_a_full_circle_with_no_straight_legs() -> None:
    parsed = parse_record(spec_examples.VERIFIED["bfwe-2-round-column"])
    segments = read_segments(parsed.geometry)
    assert len(segments) == 1
    assert segments[0].radius_mm == Decimal(300)
...
def test_a_record_may_carry_no_geometry_at_all() -> None:
    ...
    parsed = parse_record(spec_examples.VERIFIED["bfma-1-stock-mesh"])
...
def test_bar_blocks_carry_the_meshs_bars() -> None:
    parsed = parse_record(spec_examples.VERIFIED["bfma-2-drawn-mesh"])
...
def test_a_file_round_trips_byte_for_byte() -> None:
    data = spec_examples.spec_file()
    parsed = parse_file(data)
    assert len(parsed) == len(spec_examples.VERIFIED)
    assert render_file(parsed.records) == data


def test_a_trailing_newline_does_not_produce_an_empty_record() -> None:
    parsed = parse_file(spec_examples.VERIFIED["bf2d-1"] + "\r\n")
    assert len(parsed) == 1


def test_line_numbers_are_recorded_for_error_reporting() -> None:
    parsed = parse_file(spec_examples.spec_file())
    assert [record.line_no for record in parsed.records] == list(range(1, len(spec_examples.VERIFIED) + 1))


def test_ascii_bytes_are_decoded_as_ascii() -> None:
    text, encoding, offenders = decode_bytes(spec_examples.spec_file())
...
def test_a_non_ascii_byte_is_decoded_and_reported_rather_than_refused() -> None:
    ...
    record = "BF2D@HjHalle S\xfcd@r1@ia@p1@l100@n1@e0.1@d12@gB500A@s48@v@Gl100@w0@C70@"
...
def test_a_field_identifier_that_is_not_a_lowercase_letter_is_rejected() -> None:
    ...
    with pytest.raises(AbsSyntaxError, match="not a lowercase letter"):
        parse_record("BF2D@H1TestPDF@r417@C70@")
...
def test_render_block_writes_an_identifier_even_for_an_empty_value() -> None:
    assert render_block("H", [("j", "TestPDF"), ("r", "417"), ("v", "")]) == "HjTestPDF@r417@v@"


def test_render_block_marks_the_bent_axis_of_a_mesh() -> None:
    assert render_block("G", [("l", "200"), ("w", "-90")], axis="y") == "Gyl200@w-90@"
