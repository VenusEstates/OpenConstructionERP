...
Exercises the persistence layer against a real database: importing the
guideline's own test data, reading the shapes back, recognising a re-sent file,
summarising steel by bar diameter, and re-exporting bytes the bending shop can
compare against what it received.
"""
...
async def test_importing_the_guidelines_test_data_stores_every_shape(session: AsyncSession) -> None:
    project_id = await _project(session)
    service = RebarScheduleService(session)

    result = await service.import_file(project_id, "testdaten.abs", spec_examples.spec_file())

    stored = result["import_record"]
    assert result["duplicate"] is False
    assert stored.record_count == len(spec_examples.VERIFIED)
    assert stored.encoding == "ascii"

    shapes, total = await service.list_shapes(stored.id, limit=1000)
    assert total == len(spec_examples.VERIFIED)
    assert [shape.line_no for shape in shapes] == list(range(1, total + 1))
    assert all(shape.checksum_ok for shape in shapes)
    assert {shape.super_group for shape in shapes} == {"BF2D", "BF3D", "BFWE", "BFMA", "BFAU"}


async def test_header_fields_land_in_the_columns_they_belong_to(session: AsyncSession) -> None:
    project_id = await _project(session)
    service = RebarScheduleService(session)
    result = await service.import_file(project_id, "one.abs", (spec_examples.VERIFIED["bf2d-1"] + "\r\n").encode())

    shapes, _ = await service.list_shapes(result["import_record"].id)
    shape = shapes[0]
    assert shape.project_ref == "TestPDF"
    assert shape.drawing_ref == "417"
    assert shape.drawing_index == "a"
...
async def test_re_sending_the_same_file_is_recognised_rather_than_duplicated(session: AsyncSession) -> None:
    ...
    first = await service.import_file(project_id, "testdaten.abs", data)
    second = await service.import_file(project_id, "testdaten-resent.abs", data)

    assert second["duplicate"] is True
    assert second["import_record"].id == first["import_record"].id
    assert second["import_record"].filename == "testdaten.abs"
...
async def test_the_same_file_may_be_imported_into_a_second_project(session: AsyncSession) -> None:
    ...
    await service.import_file(first_project, "testdaten.abs", data)
    second = await service.import_file(second_project, "testdaten.abs", data)
...
async def test_exporting_returns_the_bytes_that_came_in(session: AsyncSession) -> None:
    ...
    service = RebarScheduleService(session)
    data = spec_examples.spec_file()
    result = await service.import_file(project_id, "testdaten.abs", data)
...
async def test_the_cutting_summary_groups_bars_and_weight_by_diameter(session: AsyncSession) -> None:
    ...
    # Nine of the guideline's BF2D shapes are 12 mm; the meshes carry no
    # diameter in the header and are excluded rather than counted as zero.
...
async def test_a_dry_run_stores_nothing(session: AsyncSession) -> None:
    ...
    preview = await service.preview(spec_examples.spec_file().decode("ascii"))

    assert preview["record_count"] == len(spec_examples.VERIFIED)
...
async def test_the_dry_run_reports_the_one_finding_the_published_data_carries(session: AsyncSession) -> None:
    """The guideline's spacer example omits two required header fields.

    Recorded here because it is the honest reading of the published data, and
    because a reader who sees the import go red should be able to find out why
